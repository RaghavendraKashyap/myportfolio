...
// ... (Simplex noise function omitted for brevity but preserved in logic below if needed, standard noise implementation)
...
void main() {
  ...
  auroraAlpha = clamp(auroraAlpha * 1.2, 0.0, 1.0);
...
const Background: React.FC<AuroraProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Use a try-catch to handle potential WebGL context creation failures silently
    ...
    const initialColors = propsRef.current.colorStops.reduce((acc, hex) => {
      const c = new Color(hex);
      acc.push(c.r, c.g, c.b);
      return acc;
    }, [] as number[]);
    ...
    const update = (t: number) => {
      ...
      if (program) {
          ...
          program.uniforms.uColorStops.value = stops.reduce((acc, hex) => {
            const c = new Color(hex);
            acc.push(c.r, c.g, c.b);
            return acc;
          }, [] as number[]);
